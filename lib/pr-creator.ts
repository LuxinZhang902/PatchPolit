  ...
  try {
    ...
    if (githubToken) {
      ...
      try {
        ...
        const prBody = `## 🤖 AI-Generated Bug Fix
...
${session.skipTests ? '⚠️ **Note:** Tests were skipped during patch generation.' : '✅ **Tests passed** after applying this patch.'}
...
*Generated by PatchPilot - AI-powered bug fixing*`;
        ...
        await prisma.debugSession.update({
          ...
          data: {
            prUrl,
            logs: (session.logs || '') + 
              `[${new Date().toISOString()}] ✅ Pull request created successfully!\n` +
        ...
        await prisma.debugSession.update({
          ...
          data: {
            prUrl: compareUrl,
            logs: (session.logs || '') + 
              `[${new Date().toISOString()}] ⚠️ Failed to create PR: ${error.message}\n` +
      ...
      await prisma.debugSession.update({
        ...
        data: {
          prUrl: compareUrl,
          logs: (session.logs || '') + 
            `[${new Date().toISOString()}] ⚠️ GITHUB_TOKEN not configured\n` +
    ...
    await prisma.debugSession.update({
      ...
      data: {
        logs: (session.logs || '') + 
          `[${new Date().toISOString()}] ✗ Failed to create pull request: ${error}\n`,
      },
    });
